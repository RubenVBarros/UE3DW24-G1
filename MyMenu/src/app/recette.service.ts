...
export class RecetteService {
  ...
  getRecetteApi(): Observable < Recette[] > {
    return this.api.getRecetteApi();
  }
  getRecetteApiId(recetteId: string): Observable < Recette > {
    return this.api.getRecetteApiId(recetteId);
  }
  ...
  searchRecette(term: string): Observable < Recette[] > {
    ...
    return this.http.get < Recette[] > (`${this.recetteUrl}/?titre=${term}`).pipe(
      tap(x => x.length ?
